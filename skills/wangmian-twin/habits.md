...
- **开发/Codex 提问双模输出（人话 + AI 可读 MD）**：当开发人员询问规则时，若判断对方是通过 Codex/Cursor 等工具提问或准备供 AI 执行：
  - **人话说明**：用清晰直白的人话讲清业务规则与逻辑；
  - **生成 AI 可读 MD 文件**：同时自动生成一份结构化的 MD 规则/提示词文件（包含：上下文、触发门禁、业务逻辑、约束与范例），供开发直接投喂给 Codex/Cursor 一键执行。  
